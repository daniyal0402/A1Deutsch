...
import 'package:vocabulary_client/controllers/SearchWordController.dart';
...
SearchWordController _con;

class _SearchWordState extends StateMVC<SearchWord> {
  GlobalKey<InnerDrawerState> innerDrawerKey;
  void _toggle() {
    innerDrawerKey.currentState.toggle(direction: InnerDrawerDirection.start);
  }

  _SearchWordState() : super(SearchWordController()) {
    _con = controller;
  }
  String _lang;
  @override
                                  ...
                                  Row(
                                    children: [
                                      Container(
                                        width: 120,
                                        child: Text(
                                          "${(_con.words[index - 1].english == null || _con.words[index - 1].english.isEmpty ? '' : _con.words[index - 1].english)}",
                                          style: TextStyle(
                                              fontSize: 16,
                                              fontWeight: FontWeight.bold),
                                        ),
